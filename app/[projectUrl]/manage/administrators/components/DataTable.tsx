...
import { Actions, InviteAdministrators } from '@/app/[projectUrl]/manage/administrators/components/buttons';
    ...
    <>
      <Container>
        <Wrapper>
          ...
          >
            <InviteAdministrators data={config.adminMaxCount} onChange={onReload} />
          </Box>
          <EmptyItem text={text} />
        </Wrapper>
      </Container>
...
const withNotExist = (WrappedComponent: React.ComponentType<Props>) => (props: Props) => {
  if (!props.data || props.data.length === 0) return <NotExist {...props} />;
...
function DataTable({ data, onReload }: Props) {
  const { i18n, t } = useTranslation();
  ...
  const { config } = useProjectStore();
  const [computedMaxCount, setComputedMaxCount] = useState(config.adminMaxCount - data.length);
      ...
      <Container>
        <Wrapper>
          ...
          >
            ...
            <InviteAdministrators data={computedMaxCount} onChange={onReload} />
          </Box>
          <Sheet
          ...
          >
            ...
            >
              <thead>
                <tr>
                  <th style={{ width: 180, padding: '12px 6px', textAlign: 'center', verticalAlign: 'middle' }}>
                    <Link
                      underline="none"
                      color="primary"
                      component="button"
              ...
              <tbody>
                {paginatedData.map((row) => (
                  <tr key={row.uid}>
                    ...
                    <td style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                      <Actions data={row} onChange={onReload} />
                    </td>
                  </tr>
                ))}
              </tbody>
