...
// data
import { PROJECTID } from '@/constants/globalDesign';
...
const Register = ({}: Props) => {
  ...
  const [selectedSpace, setSelectedSpace] = useState<'default' | 'custom'>('default');
    ...
    const getSpace = async () => {
      if (selectedSpace === 'default') {
        ...
        if (projectId === PROJECTID) {
          const res = await moduleApis.getAllPrivateSpaces();
          if (res) {
            const space = (res?.data?.spaces as any) ?? [];
            setSpaceList(space);
          }
        } else {
          const res = await moduleApis.getSpacesByProjectId(projectId);
          if (res) {
            const space = (res?.data?.spaces as any) ?? [];
            setSpaceList(space);
          }
  ...
  useEffect(() => {
    if (projectId === 'MuOU3ZnEbkcay4NQhWBq') {
      setSelectedSpace('custom');
  ...
  return (
    <>
      <Container>
        ...
        {activeStep === 0 ? (
          <>
            {projectId !== 'MuOU3ZnEbkcay4NQhWBq' && (
              <>
                <ToggleButtonGroup>
                  <ToggleButton
                    onClick={() => setSelectedSpace('default')}
                    className={selectedSpace === 'default' ? 'Mui-selected' : ''}
                  >
                    {t('기본 공간')}
                  </ToggleButton>
                  <ToggleButton
                    onClick={() => setSelectedSpace('custom')}
                    className={selectedSpace === 'custom' ? 'Mui-selected' : ''}
                  >
                    {t('나만의 공간')}
                  </ToggleButton>
                </ToggleButtonGroup>
      ...
      >
        <Box>
          ...
          <ModalTitle>[ {language === 'en' ? showcase.title_en : showcase.title} ]</ModalTitle>
