...
import DataTable from '@/app/[projectUrl]/manage/members/components/DataTable';
import { DashDateFormatter } from '@/utils/date';
import React, { useEffect, useState } from 'react';
import Loading from '@/app/loading';
import withAuth from '@/components/hoc/withAuth';
import { DENY_USER, ONLY_ENTERPRISE } from '@/constants/acess';
...
const fetchData = async (projectUrl: string) => {
  try {
    ...
    if (apiRes.meta.ok) {
      ...
      const userRes = await platformManageApis.getProjectUsers(projectId, 'user');
      const userApiRes = userRes.data as ApiResponse;

      const members = Object.values(userApiRes.data) as UserInfo[];
      ...
      <Container>
        <Header>
          ...
          >
            ...
            />
            {hover ? <HelperTxt>{t('You can manage user information and user exhibitions.')}</HelperTxt> : null}
          </HeaderTitle>
        </Header>
        <DataTable data={data} />
      </Container>
...
export default withAuth(Page, DENY_USER, ONLY_ENTERPRISE);
