...
export class User {
  ...
  static createUser(
    ...
    return new User({
      ...
      countryCode: COUNTRIES.find((country) => country.countryCode === countryCodeText)?.phoneCode || '',
      phone: phone,
  ...
  static createAdmin(
    ...
    return new User({
      ...
      assignedCount: assignedCount, // 프로젝트 config의 initialAssignedCount 와 동일
      exhibitionCount: 0,
    });
  }
  ...
  static inviteStatusUser(uid: string, projectId: string, email: string, status: string, assignedCount: number): User {
    ...
    return new User({
      ...
      survey: {
        referrer: `${status}_invite` as 'user_invite' | 'admin_invite',
        refEtcText: '',
      },
      history: {
        ip: '',
